...
class FMapsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        solicitarPermiso()
        //vamos a crear una variable para tener el mapa dentro de toda la clase
        val fragmentMapa = supportFragmentManager
            .findFragmentById(R.id.map) as SupportMapFragment
        fragmentMapa.getMapAsync{
            googleMap ->
            if(googleMap !=null){
                mapa= googleMap
                establecerConfiguracionMapa()
                val quicentro = LatLng(-0.176125, -78.480288)
                val titulo = "Quicentro"
                val zoom= 17f
                anadirMarcador(quicentro, titulo)
                moverCamaraconZoom(quicentro, zoom)
                //dibujar linea y poner etiqueta
                val poliLineaUno = googleMap
                    .addPolyline(
                        PolylineOptions()
                            .clickable(true)
                            .add(
                                LatLng(-0.1759187040647396, -78.48506472421384), //puntos para graficar la linea
